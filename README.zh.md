...
- 🈶 **中文输入法**: IME 组字预览 + 提交喵
- 📦 **手动注册**: `meowal register <名称> <路径> [-ico 图标]`,配置窗可拖入 .lnk/.exe 喵
- 🎨 **浅色/深色 + 云母/毛玻璃拟真**: 分层窗上 Skia 拟真材质喵
- 🎯 **自动聚焦最佳匹配**: 输入即搜,自动选中最匹配的应用喵
- ⌨️ **键盘全流程操作**: 方向键移动焦点、回车启动、Esc 隐藏喵
- 🖼️ **自动图标提取**: 从 exe/lnk 提取图标并缓存 PNG 快照喵
- 🏷️ **三种搜索模式**: 名称搜索、`t: 标签` 搜索、`i: 首字母` 搜索喵
- 🧮 **内置计算器**: 直接输入算式(支持 `+ - * / % ^` 与括号),回车复制结果喵
- 🌐 **Web 搜索跳转**: 任意关键词可一键用百度/必应/搜狗等引擎搜索(可配)喵
- 🖥️ **自定义浏览器**: Web 搜索可用指定浏览器打开(支持 `%1` 占位符)喵
- ⚡ **系统命令**: 指令模块支持自定义中英文别名(`commands` 配置),搜「锁屏/睡眠/关机/重启」直接执行喵
- 🖼️ **统一图标渲染**: SVG(viewBox 映射/线稿填充混排)·PNG·ICO 光栅·字符兜底,全走统一等比缩放+标准内边距喵
- 🧩 **多源搜索中枢**: SearchItem/Action 统一结果与动作模型,搜索源即插即用喵
- 🖥️ **单实例保护**: 重复启动自动唤起已有实例,不再出现双岛互踩喵
- 🎨 **浅色/深色主题**: 内置两套基础主题,完全可自定义喵
- 🗂️ **配置持久化**: 配置/注册表存于 `./.datas` 目录(JSON)喵
- ⚡ **空闲心跳**: 隐藏时 100ms 心跳排空托盘命令,可见时动画停稳降到光标闪烁间隔喵
- 🪟 **一体灵动岛**: 搜索框与结果面板合成一块超椭圆,随结果展开喵
- ⚙️ **配置 GUI**: 侧边栏 + 分组卡片设置界面,可视化配置主题/窗口/热键喵
- 🗔 **系统托盘**: 主题自适应图标 + 显示/隐藏、设置、重启、退出菜单喵
...
```
src/
├─ main.rs            # 入口: 日志、数据目录、装配多窗口+托盘、进入消息循环喵
├─ app/               # 应用级状态与生命周期喵
│  ├─ mod.rs          # AppState 状态中枢 + 共享状态 + 应用命令喵
│  └─ config.rs       # 配置结构 + JSON 持久化喵
├─ window/            # 窗口层喵
│  ├─ launcher.rs     # 启动器窗口: 状态机 + 事件分发 + 应用控制器喵
│  ├─ settings.rs     # 配置窗口: 侧边栏+分组卡片 + 交互 + 配置写回喵
│  └─ tray.rs         # 系统托盘: 图标 + 菜单 + 命令投递喵
├─ render/            # 渲染层(Skia)喵
│  ├─ mod.rs          # Renderer: Skia Surface 生命周期 + 像素输出喵
│  ├─ layout.rs       # 启动器物理像素布局计算(纯函数,可单测)喵
│  ├─ theme.rs        # 语义色板(启动器 + 配置 GUI 两套,浅色/深色)喵
│  ├─ shape.rs        # 连续曲率(G3)圆角几何喵
│  ├─ font.rs         # 字体缓存(微软雅黑)喵
│  ├─ text.rs         # 文本绘制辅助喵
│  ├─ paint.rs        # 启动器场景绘制(搜索框/面板/条目)喵
│  └─ settings.rs     # 配置 GUI 绘制(侧边栏/分组卡片/行控件)喵
├─ platform/          # 平台抽象层(唯一 cfg(target_os) 的地方)喵
│  ├─ mod.rs          # Platform trait + 平台无关事件/类型喵
│  └─ win32.rs        # 多窗口/呈现/消息泵/托盘/热键/图标/启动喵
...
3. `↑↓` 移动选中 → 条目高亮同步(超出可视区自动滚动)喵
4. `Enter` / 点击结果 → 启动应用 → 窗口隐藏喵
5. `Esc` / 失焦 → 全部隐藏喵

**多窗口协调喵**: 启动器窗口兼任「应用控制器」,托盘/配置窗口把意图投递到命令队列,启动器在定时器心跳里统一执行(呼出/打开设置/重启/退出)喵。
...
7. 共享状态用 `Rc<RefCell<AppState>>`(单线程消息循环),组件间通过命令队列解耦喵
...
6. 右键托盘 → 设置: 改主题/材质、拖入 .lnk/.exe、收藏与 Tag 喵
7. 命令行注册: `meowal register 记事本 C:\Windows\notepad.exe` 喵
...
| `search.web_engine` | `baidu` | Web 搜索引擎: `baidu`/`bing`/`sogou`/`google`/`duckduckgo` 喵 |
| `search.web_browser` | `""` | Web 搜索用的浏览器: 空=系统默认;支持含 `%1` 占位符的路径喵 |
| `search.commands` | 内置四条 | 指令模块别名表(`aliases` + `kind`),可自由增删改中英文别名喵 |
...
### ✅ 第二阶段(当前): 完整桌面体验
- [x] 系统托盘: 主题自适应图标 + 显示/隐藏、设置、重启、退出菜单喵
- [x] 配置 GUI: 侧边栏 + 分组卡片设置界面(主题/窗口/热键/搜索可视化配置)喵
- [x] 多窗口架构 + 命令队列协调喵
- [x] 中文输入法(IME)喵
- [x] 手动注册应用(命令行 `meowal register` + GUI 拖拽)喵
- [x] 毛玻璃/云母背景效果(Skia 拟真)喵
- [x] 最近使用/收藏/最常用统计展示喵
- [x] 首字母分组 + Tag 管理喵
- [x] 美化搜索框形状喵，更像灵动岛喵
...
- [x] 内置计算器(回车复制结果)喵
- [x] Web 搜索跳转(引擎可配,默认百度)喵
- [x] 系统命令(锁屏/睡眠/关机/重启)喵
- [x] 单实例互斥 + 二次启动唤起已有实例喵
- [ ] named-pipe IPC(`meowal query/show` 完整 CLI 联动)喵
- [ ] 设置 GUI RowDesc 数据驱动化(含新增配置项的可视化编辑)喵

### 🔭 未来拓展
- [ ] Everything 式文件查找喵
- [ ] QuickLook 式悬浮文件预览喵
- [ ] 插件体系 + 快捷脚本指令喵
- [ ] 变为完整灵动岛功能体系喵（如通知拦截，多媒体，日历，时间之类的喵）
- [ ] 提供桌面端全平台支持喵
