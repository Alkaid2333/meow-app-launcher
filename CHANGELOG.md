...
### Fixed
- **自定义指令的通知输出在中文系统乱码** —— 经典控制台工具（`ipconfig`、`ping` 等）在中文
  Windows 上输出 GBK（OEM 代码页 936），此前一律按 UTF-8 宽松解码，中文全变成替换符。
  现输出解码改为「UTF-8 严格校验优先，失败回落 `MultiByteToWideChar(CP_OEMCP)` 按系统
  代码页转换」，真 UTF-8 输出直通、GBK 输出正确还原，通知与日志不再乱码喵。

## [1.9.0] - 2026-09-22

### Added
- **应用扫描双向同步(增量 + 减量)** —— 应用被卸载或快捷方式从扫描目录删除后,下一次扫描会把
  对应条目从注册表 `apps.json` 里移除,图标快照与内存缓存一并清理,注册表不再留孤儿喵。
  手动注册(`Manual`)的应用不受减量影响;命中屏蔽词的条目不参与减量判断,
  加屏蔽词不会误删正常应用喵。扫描合并以 `ScanDelta`(新增 / 移除 / 移除名单)汇报,
  有增减变化时通过系统通知告知用户喵。
- **扫描热键 + 托盘入口 + 系统通知** —— 新增第二路全局热键(默认 `Ctrl+Alt+S`,可在
  「常规 · 热键」里开关与重新录制),触发后台扫描但不呼出搜索框、不抢前台;托盘菜单新增
  「扫描应用」项喵。通知能力抽象为平台特质 `NotificationSink`(接口归 `platform/`),
  Windows 用托盘气泡实现,扫描完成与指令执行结果共用这一反馈通道喵。
- **自定义指令模块** —— 指令卡片的类型按钮在 锁屏 / 睡眠 / 关机 / 重启 之外新增「自定义」档:
  选中后卡片下方渲染一条脚本输入框,写什么就跑什么;执行 shell 可在
  「常规 · 指令模块 · 指令执行 Shell」里切换(PowerShell / Cmd)喵。执行能力抽象为平台特质
  `ShellRunner`(接口归 `platform/`,Windows 实现隐藏窗口的子进程调用并截获合并输出),
  业务层只摸特质喵。
- **灵动岛指令模式** —— 输入以 `> ` 开头自动进入指令模式: 结果面板只显示一条「执行指令」,
  回车交由配置的 shell **后台线程**执行,不卡 UI;完成后经系统通知反馈输出(成功 / 失败),
  长任务也不会闪黑窗喵。

### Changed
- 管理中心与托盘 / 热键触发的扫描共用同一条「扫描 → 合并 → 通知」链路;`CommandEntry` 结构新增
  `script` 字段(旧配置缺字段自动回填空串),系统命令类型枚举新增 `custom` 档喵。

### Fixed
- **管理中心搜索后点击别处会清空搜索词** —— 过滤词此前只存在编辑草稿里,点击输入框外的任意位置
  草稿被直接丢弃,搜索词与结果一起消失;现在过滤词有独立的持久字段,点击别处只是提交并收起
  光标,搜索词与过滤结果原样保留喵。
- **管理中心搜索后视口停在旧位置导致"看不到结果"** —— 先向下滚动再搜索时,结果集变矮但滚动
  位置不变,内容被甩出视野上方,看起来像"没结果";现在过滤词每变化一次视口自动回到顶部,
  且每帧把滚动目标钳回内容范围内兜底喵。

## [1.8.0] - 2026-09-17

### Added
- **按住修饰键提权启动** —— 呼出后按住指定修饰键再回车 / 点击结果,即以管理员身份运行该应用
  (Windows 走 `ShellExecuteW` + `runas` 动词触发 UAC)。修饰键可在「常规 · 热键」里切换
  Shift(默认)/ Ctrl / Alt / Win,或整体关闭;用户在 UAC 弹窗上点「否」只记一条 warn,
  不当错误处理喵。权限提升是**平台能力**,已抽象为 `launch_elevated` + 平台无关的 `Modifier` /
  `ModifierSet`,未来 Linux 后端对应 `sudo` 即可复用同一套业务逻辑喵。
- **常驻进程环境变量自动同步** —— 本应用常驻不重启,子进程继承的却是「启动那一刻」的环境块快照,
  用户在系统属性里改完 PATH 后,从本应用打开 Terminal 仍会用旧值。现同步修复:
  收到 `WM_SETTINGCHANGE`(`lParam = Environment`)立刻重读系统键与用户键下的环境变量,
  两级合并 → 展开 `%引用%` → 差量写回本进程环境块;启动时与每次启动应用前也会对齐一次,
  子进程继承的永远是最新环境喵。
  合并规则与 Windows 登录环境一致: 同名时用户值覆盖系统值,**`PATH` 例外(用户段拼接在系统段之后,
  绝不丢系统 PATH)**;`TEMP` / `TMP` 属会话级变量,只有用户键显式给出时才采用喵。

### Changed
- **岛体动效更灵动** —— 结果列表新增「级联入场」: 呼出与展开时各行按错峰节奏向上浮现
  (错峰进度由纯函数 `island::cascade` 计算,带单测),收起时直接定型避免淡出叠影;
  降低动效或瞬时档位下不播放,无障碍优先喵。
- **结果滚动改为平滑趋近** —— 滚轮与键盘导航只更新目标位置,心跳里按帧率无关的指数平滑推进
  (`springs::smooth_toward`,恒定 dt 下可结合);滚动条拖动仍即时跟手,停止时按阈值吸附,
  不留无穷尾巴喵。
- **新增配置项 `elevate_modifier`** —— `disabled` / `shift`(默认)/ `ctrl` / `alt` / `win`,
  经 `RowDesc` 注册表进入「常规 · 热键」分组,配置 GUI 循环切换、即时生效喵。

### Fixed
- **从常驻实例启动的应用拿到陈旧环境变量** —— 见上面「常驻进程环境变量自动同步」喵。
